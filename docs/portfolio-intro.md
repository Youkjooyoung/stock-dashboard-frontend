...
국내 주식 정보를 한 화면에서 조회·분석하고, 포트폴리오를 직접 관리할 수 있는 웹 대시보드가 필요했습니다. 단순 CRUD를 넘어 **실시간 시세(WebSocket)**, **소셜 로그인(OAuth2)**, **AI 종목 분석(OpenAI API)**, **보안 본인인증(PortOne V1)** 등 실무 수준의 기술을 직접 설계·구현하며 풀스택 역량을 증명하는 것이 목표였습니다.
...
**주식 기능**: 시세 조회/즐겨찾기, 포트폴리오 관리, 목표가 알림, WebSocket 실시간 시세, AI 종목 분석(OpenAI API), 뉴스 조회
