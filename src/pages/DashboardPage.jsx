...
export default function DashboardPage() {
  ...
  // WebSocket ??/topic/prices Íµ¨ÎèÖ: Î∞±Ïóî???òÏßë ?ÑÎ£å ??Ï∫êÏãú Î¨¥Ìö®??  useEffect(() => {
    const client = new Client({
      ...
      onStompError: (frame) => {
        console.warn('[WebSocket] STOMP ?§Î•ò:', frame.headers?.message);
      },
      onDisconnect: () => {
        console.info('[WebSocket] ?∞Í≤∞ ?äÍ? ??5Ï¥????¨Ïó∞Í≤??úÎèÑ');
      },
      onWebSocketError: (e) => {
        console.warn('[WebSocket] ?åÏºì ?§Î•ò:', e);
      },
    });
    client.activate();
  ...
  // ?êÎèôÍ∞±Ïã† ?∏ÌÑ∞Î≤?(WebSocket ÎØ∏Ïó∞Í≤????¥Î∞±)
  useEffect(() => {
    if (autoRefresh) {
  ...
  // ÎßàÏö¥?????òÏßë???¥Î? ÏßÑÌñâ Ï§ëÏù¥Î©??êÎèô?ºÎ°ú ?¥ÎßÅ ?úÏûë
  useEffect(() => {
      ...
      <div className={styles['bulk-bar']}>
        {bulkStatus && bulkStatus.status === 'running' ? (
          <div className={styles['bulk-progress']}>
            ...
            <span>
              ?ÑÏ≤¥ Í≥ºÍ±∞ ?∞Ïù¥???òÏßë Ï§ë‚Ä?nbsp;
              {bulkStatus.total > 0
                ? `${bulkStatus.current} / ${bulkStatus.total} Ï¢ÖÎ™©`
                : 'Ï§ÄÎπ?Ï§?}
            </span>
          </div>
        ) : bulkStatus && bulkStatus.status === 'done' ? (
          <div className={styles['bulk-done']}>
            ???ÑÏ≤¥ ?òÏßë ?ÑÎ£å ({bulkStatus.total}Ï¢ÖÎ™©)
          ...
          <div className={styles['bulk-wrap']}>
            ...
            {bulkOpen && (
              <div className={styles['bulk-dropdown']}>
                <div className={styles['bulk-dropdown-title']}>?ÑÏ≤¥ ?òÏßë</div>
                {[1, 3, 5, 10].map(y => (
                  <button key={y} className={styles['bulk-option']} onClick={() => startBulkCollect(y, false)}>
                    {y}??({y * 252}Í±∞Îûò??
                  </button>
                ))}
                <div className={styles['bulk-dropdown-title']} style={{ marginTop: 8 }}>ÎØ∏ÏàòÏß?Ï¢ÖÎ™©Îß?/div>
                {[1, 3, 5, 10].map(y => (
