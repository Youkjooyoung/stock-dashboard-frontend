...
export default function StockChat({ ticker, stockName }) {
  ...
  const nickname = localStorage.getItem('kakaoNickname') || email.split('@')[0] || '?µÎ™Ö';
  ...
  return (
    <div className={styles['chat-wrap']}>
      ...
      <div className={styles['chat-header']}>
        <span className={styles['chat-title']}>?í¨ {stockName} ?†Î°†</span>
        <span className={`${styles['chat-status-dot']} ${connected ? styles.connected : styles.disconnected}`} />
        <span className={styles['chat-status-text']}>{connected ? '?∞Í≤∞?? : '?∞Í≤∞ Ï§?..'}</span>
      </div>

      {/* Î©îÏãúÏßÄ Î™©Î°ù */}
      <div className={styles['chat-message-list']}>
        {messages.length === 0 && (
          <div className={styles['chat-empty']}>Ï≤?Î≤àÏß∏ ?òÍ≤¨???®Í≤®Î≥¥ÏÑ∏??</div>
        )}
        {messages.map((msg, i) => {
      ...
      <div className={styles['chat-input-row']}>
        <input
          ...
          placeholder="?òÍ≤¨???ÖÎ†•?òÏÑ∏??.. (EnterÎ°??ÑÏÜ°)"
          value={input}
